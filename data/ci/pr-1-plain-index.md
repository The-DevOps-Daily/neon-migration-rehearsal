...
0 of 1 passed, run in order on one branch. Blocking limit: 1000 ms.

| Migration | Result | Ran for | Runs | App | Blocking | Rows |
|---|---|---|---|---|---|---|
| `007_index_orders_status.sql` | FAIL | 4.1 s | ok: completed | ok: no app query failed | **fail**: worst write 3815 ms, worst read 126 ms (limit 1000 ms) | ok: no table lost rows |

<details><summary>Schema diff: 007_index_orders_status.sql</summary>

```diff
--- Database: neondb	(Branch: br-ancient-salad-b2vntgbd)
+++ Database: neondb	(Branch: br-calm-violet-b2yk8w89)
@@ -124,8 +124,15 @@
